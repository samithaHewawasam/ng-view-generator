...
echo json_encode(array("commitCode" => false, "errorInfo" => 'There is an error in receipt no'));
...
if (isset($_GET['RG_Reg_N0']) && $responseRegistration) {
    ...
    $RG_Reg_N0 = $_GET['RG_Reg_N0'];

}else{

echo json_encode(array("commitCode" => false, "errorInfo" => 'invalid registration number'));
...
VALUES (?,?,?,?,?,?,?,?,?)",
        'bind' => array($InvoiceNumber,$RG_Reg_N0,$RG_Date,$currency,$currencyRate,$PM_Amount_Cash,$excessPayment,$PM_Type,$PM_Operator)
    ), 
    array(
        'query' => "UPDATE `registrations` SET `RG_Total_Paid`=? WHERE `RG_Reg_NO`=?",
        'bind' => array($nextTotalPaidUpdate,$RG_Reg_N0)
    ),
    array(
...
$arguments = array(
    array(
        ...
        'bind' => array($InvoiceNumber, $RG_Reg_N0, $RG_Date, $currency ,$currencyRate, $PM_Amount_credit, $excessPayment, $PM_Type,$PM_Card_Holder_Name, $PM_Card_Type, $PM_Card_NO, $PM_Operator)
    ), 
    array(
        'query' => "UPDATE `registrations` SET `RG_Total_Paid`=? WHERE `RG_Reg_NO`=?",
        'bind' => array($nextTotalPaidUpdate,$RG_Reg_N0)
...
$arguments = array(
    array(
        ...
        'bind' => array($InvoiceNumber, $RG_Reg_N0, $RG_Date, $currency, $currencyRate, $PM_Amount_Cheque, $excessPayment, $PM_Type,$PM_Cheque_NO, $PM_Cheque_Bank, $PM_Cheque_Due_Date, $PM_Operator)
    ), 
...
foreach ($rows as $r) {
   ...
   array_push($arguments, array('query' => "UPDATE `student_installments` SET `SI_Paid_Amount` = ?, `SI_Paid_Date` = ? WHERE `SI_Ins_NO` = ? AND `SI_Reg_No` =?" , 'bind' => array($paying,$RG_Date,$r['SI_Ins_NO'],$RG_Reg_NO) ));
