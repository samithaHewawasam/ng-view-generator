...
$SearchFrom='Registration NO';
 $sql="SELECT * FROM `registrations` WHERE `RG_Reg_NO` LIKE ? LIMIT 50";
...
{
echo '<h3>Can\'nt Find Results For this Registration No or part</h3>';
}
}
...
$sql="SELECT * FROM `student_master` $wherecol LIMIT 50";
 $STH=$esoftConfig->prepare($sql);
 $STH->execute($data);
 $result = $STH->fetchAll(PDO::FETCH_ASSOC);
if($STH->rowCount()){
...
{
echo '<h3>Can\'nt Find Results For this ID Number  or part</h3>';
}
...
echo '<li ><a href="#tab'.$i.'" data-toggle="tab">'.$row['RG_Reg_NO'].'</a></li>';
}
echo '  </ul>';
...
{
echo '<h3>No Registrations for this student.</h3>';
}
