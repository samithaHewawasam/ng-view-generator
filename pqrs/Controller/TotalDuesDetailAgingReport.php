...
break;
case 5:
$having=' HAVING `E` BETWEEN 31 AND 60';
break;
case 6:
$having=' HAVING `E` > 60';
break;
...
$z++;
 //$SqlSetOne = $SqlMain.$FirstRangeWhere.$SqlTail.$having;
  $SqlSetOne = $SqlMain." WHERE 1 AND `registrations`.`RG_Reg_NO` ='".$v."' AND `student_installments`.`SI_Due_Date` <= CURDATE()  AND (`SI_Ins_Amount`- `SI_Paid_Amount`) > 0 
";	
...
$arrayforexel[$row['Currency']][0]=array('Reg No','Stu ID','Student Name','Tell(TM/TR/TP)','Student E-mail','Batch','Total Paid','Due Date','Due Age','Upto Date Dues','Last Pay');	
$i=1;
	}
$NewCur=$row['Currency'];		
$StudentName=$row['SM_Title'].' '.$row['SM_First_Name'].' '.$row['SM_Last_Name'];
$DueAge=empty($row['D'])? '':floor(($Today-strtotime($row['D']))/86400);

$arrayforexel[$row['Currency']][$z]=array($row['RG_Reg_NO'],$row['SM_ID'],$StudentName,$tel,$row['SM_Mail_Personal'],$row['Default_Batch'],$row['RG_Total_Paid'],$row['D'],$DueAge,$row['C'],$row['P']);	
...
$ResultTable.=  '<tr '.$class.'>
	...
	<td>'.$roww[7].'</td>
	<td>'.$roww[8].'</td>
    <td><div class="text-right" >'.$roww[9].'</div></td>
    <td><div class="text-right" >'.$roww[10].'</div></td>
    <td></td>
  </tr>';
  $i++;	
}
...
<div class="row">	
<div class="col-md-12">
<h4>Due Aging Detail Report - '.$bA[0].'</h4>
