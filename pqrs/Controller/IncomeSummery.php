...
if (!empty($_POST['BranchSelect']) and $_POST['BranchSelect']!='All' && $_SESSION['bset']==1){
$_POST['SelectedBranch']=null;
$_POST['SelectedBranch'][$_POST[$_SESSION['Sys_U_Branches']]]=$_POST['BranchSelect'];
}
		...
		if ($count2)
          ...
          if(empty($row['RC2'])){
				...
				$RC2=$row['RC2'];
							...
							$TotalOfRang2[]=$RC2;
							$F=($RC1-$RC2);
                 $table.= '<tr><td>' . $BranchName . '</td><td><div class="text-right" >' . number_format($RC1,2) . '</div></td><td><div class="text-right" >' . number_format($RC2,2) . '</div></td><td><div class="text-right" style="'.($F >0 ? 'color:#33CC00':'color:#CC0000').'" >'. number_format( $F, 2).'</div></td></tr>';
...
<table class="table" >
  ...
  <tr class="warning">
	...
	echo '</table>
     ...
     <div class="col-md-6">
<table class="table tablesorter table-hover" id="myTablex"  >
<thead>
    ...
    <td><div class="text-right" style="'.($T >0 ? 'color:#33CC00':'color:#CC0000').'" >'. number_format($T , 2).'</div></td>
  </tr></table>
  </div>
...
$DataBase=$DBprefix.(strtolower($Branch)).$DBsuffix;
$SQLSYNC="SELECT `date` FROM $DataBase.`get_sync` WHERE Sync_ID LIKE '%".$Branch."%' AND `status`='1' ORDER BY `id` DESC LIMIT 1 ";
...
if($TypesOfBranchesMain){
	...
	foreach($TypesOfBranches as $b => $C)
		...
		if (!empty($TotalOfBranchesMain[$cur][$b]))
			...
			$arrayStackedchart[$bA[0]] = $totalbranch;
...
<td>'.$LastSync[$bA[1]].'</td>		';
 ...
 <div class="row">
	 ...
	 <h4>Bar Chart  Analyze</h4>
	 <div id="ChartDiv" ChartName="BarChart">';
		
	 
//<span class="Chartdata hide">[["Branch", "Amount"],' . implode(',', $arraychart) . ']</span>
echo '<span class="Chartdata hide">[
        ["Genre","'.implode('","', array_keys($arrayStackedchart)).'", { "role": "annotation" } ],
        ["", ' . implode(',', $arrayStackedchart) . ',""]
       
      ]</span>
<span class="Options hide">
{ "title": "Branch Wise Payment Collection " , "height": "200","width": "900","legend": { "position": "top", "maxLines": "3" },
	"bar": { "groupWidth": "30%" },
        "isStacked": "true" }
</span>
</div>
