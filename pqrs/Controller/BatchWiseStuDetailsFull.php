...
$bA[0]=	$BranchName;	
$bA[1]=	$key;
 $myReport = "temp/StuDetails".$bA[0].".xlsx";

$DataBase = $DBprefix. strtolower($bA[1]).$DBsuffix;
$DateTable=" `registrations`.`RG_Date` ";
include('SearchPostPart.php');
$CoursPart=" AND `course`.`C_Code` IN('DITEC-INT','DITEC','DITEC-I','DITEC-ONLY','DIP-ENGLISH','DUAL-DIP')";
$DivisionPart=null;
$BatchPart=null;
$BatchStatusPart=null;
$FirstRangeWhere = $Where.$RGStatusPart.$DivisionPart.$BatchPart. $BatchStatusPart.$CoursPart.$IntakePart; 
$SqlMain = "SELECT `course`.`C_Code`,`registrations`.`RG_Final_Fee`,`registrations`.`RG_Total_Paid`,`registrations`.`RG_Reg_NO`,`registrations`.`RG_Reg_Type`,`registrations`.`RG_Date`,`registrations`.`RG_Status`,`student_master`.`SM_ID`,`student_master`.`SM_Title`,`student_master`.`SM_First_Name`,`student_master`.`SM_Last_Name`,`student_master`.`SM_Tell_Mobile`,`student_master`.`SM_Tel_Residance` ,`student_master`.`SM_Mail_Personal`,`student_master`.`SM_Parent_Phone`,`student_master`.`SM_House_NO`,`student_master`.`SM_Gender`,`student_master`.`SM_Date_of_Birth` FROM $DataBase.`registrations`
...
$arrayforexel=array();
$arrayfor=array('No','Branch','Stu ID','Reg No','Reg Type','Reg Date','Title','SM_First_Name','SM_Last_Name','Gender','DOB','Status','Final Fee','Total Paid');	
/*$ResultTable.='
...
$arrayforexel[$row['C_Code']][]=array($i++,$bA[0],$row['SM_ID'],$row['RG_Reg_NO'],$row['RG_Reg_Type'],$row['RG_Date'],$row['SM_Title'],$row['SM_First_Name'],$row['SM_Last_Name'],$row['SM_Gender'],$row['SM_Date_of_Birth'],$row['RG_Status'],$row['RG_Final_Fee'],$row['RG_Total_Paid']);			
/*$ResultTable.=  '<tr >
    <td>'.$i++.'</td>
    <td>'.$bA[0].'</td>
    <td>'.$row['SM_ID'].'</td>
...
$re.='<a href="Controller/'.$myReport.'">'.$bA[0].'Export to Exel</a><br />
<br />
';
}
