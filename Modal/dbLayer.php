...
class dbLayer
{
    ...
    {
        $sql = $this->done->prepare($sql);
        $sql->execute();
    }
    
    public function prepareSelected(array $data)
    {
        
        $sql = $this->done->prepare("SELECT * FROM `system_users` WHERE `Sys_U_Username` =? AND `Sys_U_Password` =?");
        $sql->execute($data);
        return $sql->fetchALL(PDO::FETCH_OBJ);
    }
    ...
    public function CourseTypeList($branch)
    {
        ...
        $sql = $this->done->prepare("SELECT DISTINCT `C_Code`,`C_Ins_Method` from `course` WHERE C_Status = 'Active' ORDER BY `C_Code` ASC");
        $sql->execute(array(
            $branch,
            "ALL",
            "Active"
    ...
    {
        $sql = $this->done->prepare("SELECT `CT_Type_Code` FROM `course_type` INNER JOIN `registration_type` ON `registration_type`.`RT_Code` = `course_type`.`CT_Type_Code`  WHERE CT_Course_Code= ? AND  `RT_Status` = 'Active' ");
        $sql->execute(array(
            $CT_Course_Code
        ));
        return $sql->fetchALL(PDO::FETCH_OBJ);
    ...
    {
        $sql = $this->done->prepare("SELECT `S_Code`,`S_Name`,`C_Code`,`S_Status` FROM `subjects` WHERE C_Code=? AND S_Status = 'Active'");
        $sql->execute(array(
            $C_Code
        ));
    ...
    //SubjectsSet from here 
    public function doManualSync()
    {
        $sql = $this->done->prepare("SELECT `query`,`data`,`Sync_ID` FROM `sync_log` WHERE `Sync_Status` IS NULL");
        $sql->execute();
        return $sql->fetchALL(PDO::FETCH_ASSOC);
    }
    ...
    {
        $sql = $this->done->prepare("SELECT `Bm_Batch_Code`,`BM_Commence_Date`,`BM_End_Date`,`BM_Ins_Days`,(SELECT GROUP_CONCAT(`dayName`) FROM `batch_schedule` WHERE `batch` = `Bm_Batch_Code`) schedules FROM `batch_master` WHERE BM_Course_Code=? AND BM_Status=?");
        $sql->execute(array(
            $BM_Course_Code,
            "Active"
        ));
    ...
    {       
        ...
        return array(
            'returnSyncID' => $arguments['Sync_ID'],
            'errorInfo' => $this->errorInfo,
            'commitCode' => $this->commitCode,
            'roalBackCode' => $this->roalBackCode
        );
...
ORDER BY `PM_Receipt_No` DESC LIMIT 1");
    ...
    {
        $sql = $this->done->prepare("SELECT * FROM `sync_log` WHERE Sync_Status IS NULL ORDER BY Sync_ID ASC LIMIT 1");
        $sql->execute();
        return $sql->fetchALL(PDO::FETCH_ASSOC);
